...
exports.searchByText = async (req, res) => {
    try {
        ...
        const results = await openFoodFactsService.searchByText(query.trim(), pageNum, pageSize);
        res.json({
            success: true,
            query: query.trim(),
            ...results,
...
exports.searchByCategory = async (req, res) => {
    try {
        ...
        const results = await openFoodFactsService.searchByCategory(category, pageNum, pageSize);
        res.json({
            success: true,
            category,
            ...results,
...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        const suggestions = await openFoodFactsService.getProductSuggestions(query.trim(), Math.min(parseInt(limit) || 10, 20));
        res.json({
            success: true,
            query: query.trim(),
            suggestions,
...
exports.addToFoodItems = async (req, res) => {
    try {
        const { barcode } = req.params;
        const { location = "shopping-list", quantity = 1, unit = "pcs", shoppingListId = null, mealId = null, } = req.body;
        if (!barcode) {
            return res.status(400).json({
                success: false,
                message: "Barcode is required",
        ...
        const foodItemData = {
            ...
            openFoodFactsData: {
                ...
                nutritionGrade: ["a", "b", "c", "d", "e"].includes(offProduct.nutritionGrade?.toLowerCase())
