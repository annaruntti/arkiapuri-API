...
exports.createFoodItem = async (req, res) => {
    try {
        const { name, category, quantity, price, calories, location, listId, expirationDate, unit, quantities: requestQuantities, locations, } = req.body;
        const foodItem = new FoodItem({
            ...
            quantities: {
                ...
                "shopping-list": location === "shopping-list"
                    ? parseFloat(quantity) || 0
...
exports.updateFoodItem = async (req, res) => {
    try {
        const foodItem = await FoodItem.findOneAndUpdate({ _id: req.params.id, user: req.user._id }, req.body, { new: true });
        if (!foodItem) {
...
exports.moveFoodItem = async (req, res) => {
    try {
        ...
        const foodItem = await FoodItem.findOneAndUpdate({ _id: id, user: req.user._id }, {
            location,
            listId: listId || null,
    ...
    try {
        ...
        switch (action) {
            case "add":
                ...
                break;
            case "subtract":
                foodItem.quantities[location] = Math.max(0, foodItem.quantities[location] - quantity);
                break;
            case "set":
...
exports.moveItem = async (req, res) => {
    try {
        ...
        if (!["meal", "shopping-list", "pantry"].includes(fromLocation) ||
...
exports.uploadFoodItemImage = async (req, res) => {
    try {
        ...
        if (!process.env.CLOUDINARY_USER_NAME ||
...
exports.findOrCreateFoodItem = async (req, res) => {
    try {
        const { name, category, unit, price, calories, location, quantities } = req.body;
        if (!name) {
            return res.status(400).json({
                success: false,
        ...
        if (!existingFoodItem) {
            ...
            for (const item of allFoodItems) {
                ...
                const similarity = calculateSimilarity(normalizedSearchName, normalizedItemName);
        ...
        if (pantry && pantry.items) {
            for (const pantryItem of pantry.items) {
                if (pantryItem.foodId) {
                    ...
                    if (pantryItemName === normalizedSearchName ||
            ...
            if (list.items) {
                for (const listItem of list.items) {
                    if (listItem.foodId) {
                        ...
                        if (listItemName === normalizedSearchName ||
