...
exports.updateShoppingList = async (req, res) => {
    try {
        ...
        await ShoppingList.findOneAndUpdate({ _id: id, userId: req.user._id }, {
            items,
            totalEstimatedPrice,
        }, { new: true });
...
exports.addItemsToShoppingList = async (req, res) => {
    try {
        ...
        shoppingList.totalEstimatedPrice = shoppingList.items.reduce((total, item) => total + (item.estimatedPrice || 0), 0);
