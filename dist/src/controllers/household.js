...
exports.leaveHousehold = async (req, res) => {
    try {
        ...
        if (household.owner.toString() === userId.toString()) {
            return res.status(400).json({
                success: false,
                message: "Omistaja ei voi poistua perheestä. Siirrä omistajuus toiselle jäsenelle tai poista perhe.",
            });
        }
        // Remove user from household members
        household.members = household.members.filter((member) => member.userId.toString() !== userId.toString());
        await household.save();
        // Update user's household reference
        await User.findByIdAndUpdate(userId, { household: null });
        res.json({
...
exports.removeMember = async (req, res) => {
    try {
        ...
        // Remove member
        household.members = household.members.filter((member) => member.userId.toString() !== memberId);
        await household.save();
        // Update user's household reference
...
exports.acceptInvitation = async (req, res) => {
    try {
        ...
        if (invitation.email !== req.user.email.toLowerCase()) {
            console.warn(`User ${req.user.email} accepting invitation for ${invitation.email}`);
        }
