...
const fs = require("fs");
const { getDataOwnership, getDataQuery, } = require("../helpers/householdHelpers");
exports.createMeal = async (req, res) => {
    try {
        const { name, recipe, difficultyLevel, cookingTime, foodItems, defaultRoles, mealCategory, plannedCookingDate, plannedEatingDates, createdAt, } = req.body;
        ...
        if (invalidRoles.length > 0) {
            return res.status(400).json({
                success: false,
                message: `Invalid roles found: ${invalidRoles.join(", ")}. Allowed roles are: ${validRoles.join(", ")}`,
            });
        }
        ...
        const populatedMeal = await Meal.findById(meal._id).populate({
            path: "foodItems",
            select: "name quantity unit category calories price location quantities locations",
        });
...
exports.updateMeal = async (req, res) => {
    try {
        ...
        // Update the meal
        const updatedMeal = await Meal.findOneAndUpdate({ _id: mealId, user: req.user._id }, updateData, { new: true }).populate({
            path: "foodItems",
            select: "name quantity unit category calories price location quantities locations",
        });
